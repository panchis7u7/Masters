...
\begin{center}
    \textbf{Participantes Equipo 1092 E}\\[0.2cm]
    José Agustín Miranda Rentería\\
    Gino Callán Aguilar\\
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Recuperación de Entidades Sensibles}: Qué porcentaje de entidades sensibles fueron correctamente identificadas y anonimizadas.
        \item \textbf{Tasa de Falsos Positivos/Negativos}: Qué porcentaje de entidades sensibles fueron incorrectamente identificadas o no detectadas.
    \end{itemize}
    Calidad de extracción de entidades:

    \textbf{Precisión}: Qué porcentaje de entidades extraídas son correctas. Generalmente se busca un 80% o más de precisión para considerar que el modelo es confiable.

    \textbf{Recuperación de Entidades Significativas}: Qué porcentaje de entidades correctas fueron extraídas.

    \textbf{Puntuación F1}: Medida combinada de precisión y la recuperación de entidades significativas. Los rangos de interpretación son los siguientes:
...
El LLM operará bajo una arquitectura de Recuperación Aumentada por Generación (RAG), en la que la generación se condiciona con evidencia documental recuperada desde el índice vectorial para reducir alucinaciones y mejorar trazabilidad.

Para la generación de representaciones vectoriales se utilizará un único modelo de embeddings y una base de datos vectorial dedicada, manteniendo fija la misma versión del modelo, dimensión de vector y métrica de similitud (coseno) durante todo el experimento para garantizar la comparabilidad entre corridas. Se propone \textbf{BAAI/bge-m3} (NVIDIA, 2026) por su alto desempeño en recuperación semántica multilingüe (incluyendo español técnico-legal), su robustez en documentos largos y su estabilidad para indexación consistente en escenarios RAG.

Para el componente generativo y de razonamiento del expediente, se utilizará \textbf{LLaMA 3} como modelo base. En inferencia, cada predicción incorporará: (i) top-$k$ fragmentos recuperados del expediente del proveedor, (ii) plantilla de instrucción jurídica-operativa y (iii) salida estructurada para consumo del clasificador de riesgo.
...
El modelo de clasificación de riesgo se implementará con \textbf{LLaMA 3 + RAG} y ajuste fino supervisado eficiente (\textit{parameter-efficient fine-tuning}). El plan de \textit{fine-tuning} será: (1) curación y balanceo de ejemplos etiquetados Verde/Amarillo/Rojo, (2) ajuste LoRA/QLoRA sobre LLaMA 3 con congelamiento del modelo base, (3) validación estratificada y calibración de umbrales de riesgo, y (4) evaluación final contra \textit{ground truth} experto con métricas de Accuracy, F1 y Cohen's Kappa. Este enfoque permite especializar el clasificador al dominio contractual de El Cuino sin reentrenamiento completo, controlando costo computacional y manteniendo la extensión objetivo del documento (28 páginas).
...
\begin{enumerate}
    ...
    \item \textbf{Modelo de Embeddings Seleccionado para Recuperación Semántica}

    NVIDIA. (2026). \textit{BAAI/bge-m3 - Model Card}. NVIDIA Build. \newline
    \textit{URL:} \url{https://build.nvidia.com/baai/bge-m3/modelcard}
